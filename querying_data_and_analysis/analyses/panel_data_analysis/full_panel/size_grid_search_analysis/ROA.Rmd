...
while (start_upper + step_size * i < largest_bank){
    ...
    tryCatch({
        ...
        t1_coef <- result1$coefficients[rownames(result1$coefficients) == "t1_LR_lagged_1_year"][1]
        t1_aa_coef <- result1$coefficients[rownames(result1$coefficients) == "I(black_ind * t1_LR_lagged_1_year)"][1]
        t1_his_coef <- result1$coefficients[rownames(result1$coefficients) == "I(hispanic_ind * t1_LR_lagged_1_year)"][1]

        total_t1_aa <- t1_coef + t1_aa_coef
        total_t1_his <- t1_coef + t1_his_coef


        sb_coef <- result1$coefficients[rownames(result1$coefficients) == "less_100_lagged_SB_loans_TA"][1]
        sb_aa_coef <- result1$coefficients[rownames(result1$coefficients) == "I(black_ind * less_100_lagged_SB_loans_TA)"][1]
        sb_his_coef <- result1$coefficients[rownames(result1$coefficients) == "I(hispanic_ind * less_100_lagged_SB_loans_TA)"][1]

        total_sb_aa <- sb_coef + sb_aa_coef
        total_sb_his <- sb_coef + sb_his_coef
    },
