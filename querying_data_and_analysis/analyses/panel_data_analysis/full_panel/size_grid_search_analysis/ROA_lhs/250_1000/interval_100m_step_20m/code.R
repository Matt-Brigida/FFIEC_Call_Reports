...
while (start_upper + step_size * i < largest_bank){
    ...
    tryCatch({
        FEmodel1 <- plm(ROA ~  amt_CI_250_1000_SB_loans_Delt_lagged_1_year + t1_LR_lagged_1_year + X250_1000_lagged_SB_loans_TA + NPA_TA + TD_TA + I(post_crisis_ind) + I(black_ind * t1_LR_lagged_1_year) + I(hispanic_ind * t1_LR_lagged_1_year) + I(black_ind * X250_1000_lagged_SB_loans_TA) + I(hispanic_ind * X250_1000_lagged_SB_loans_TA), data = panel, model = "within", effect = "twoways")
        ...
        sb_coef[i] <- result1$coefficients[rownames(result1$coefficients) == "X250_1000_lagged_SB_loans_TA"][1]
        sb_aa_coef[i] <- result1$coefficients[rownames(result1$coefficients) == "I(black_ind * X250_1000_lagged_SB_loans_TA)"][1]
        sb_his_coef[i] <- result1$coefficients[rownames(result1$coefficients) == "I(hispanic_ind * X250_1000_lagged_SB_loans_TA)"][1]
...
for (k in 1:length(t1_coef)){
          xaxis[k] <- (interval_size + step_size * (k - 1)) * 1000
}
