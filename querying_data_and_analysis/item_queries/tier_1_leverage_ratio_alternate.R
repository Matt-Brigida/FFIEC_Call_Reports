...
query_t1_leverage_ratio_alternate <- function(ID){
    ...
    code <- ""
    code[i] <- ifelse(i < 57, "RCON7204", "RCOAP793")

    t1_leverage_ratio_alternate[i] <- if( length(as.numeric(eval(parse(text = paste0("sch$", code[i]))))) == 0){
                       NA
                   } else {
    ...
    result <- xts::as.xts(as.numeric(t1_leverage_ratio_alternate), order.by = rept_date)
    names(result) <- "t1_leverage_ratio_alternate"
    return(result)
}
