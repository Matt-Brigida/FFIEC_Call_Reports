...
query_t1_leverage_ratio <- function(ID){
    ...
    code <- ""
    code[i] <- ifelse(i < 57, "RCON7204", "RCOA7204")

    t1_leverage_ratio[i] <- if( length(as.numeric(eval(parse(text = paste0("sch$", code[i]))))) == 0){
                       NA
                   } else {
    ...
    result <- xts::as.xts(as.numeric(t1_leverage_ratio), order.by = rept_date)
    names(result) <- "t1_leverage_ratio"
    return(result)
}
