...
query_npa_nonacc <- function(ID){
    ...
    # npa_nonacc[j] <- as.numeric(sch$RCON1606)
    npa_nonacc[j] <- if( length(as.numeric(eval(parse(text = paste0("sch$", code))))) == 0){
                       NA
                   } else {
    ...
    result <- xts::as.xts(as.numeric(npa_nonacc), order.by = rept_date)
    names(result) <- "npa_nonacc"
    return(result)
}
