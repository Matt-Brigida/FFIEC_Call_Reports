...
query_npa_30_89 <- function(ID){
    ...
    # npa_30_89[j] <- as.numeric(sch$RCON1606)
    npa_30_89[j] <- if( length(as.numeric(eval(parse(text = paste0("sch$", code))))) == 0){
                       NA
                   } else {
    ...
    result <- xts::as.xts(as.numeric(npa_30_89), order.by = rept_date)
    names(result) <- "npa_30_89"
    return(result)
}
